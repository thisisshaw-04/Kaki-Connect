...
export function AppShell({
  ...
  return (
    <div className={cn("relative flex h-full min-h-0 min-w-0 flex-col bg-background", className)}>
      ...
      <main className="flex min-h-0 min-w-0 flex-1 flex-col overflow-y-auto px-5 py-2">
        {children}
      </main>
      {footer ? (
        <div className="relative z-20 shrink-0 bg-background px-5 pt-1 pb-2">{footer}</div>
      ) : null}
      {showNav ? (
        <nav
